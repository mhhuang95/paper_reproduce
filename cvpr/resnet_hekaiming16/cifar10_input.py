...
def maybe_download_and_extract():
    '''
    download and extract the cifar10 data automatically
    :return:
    '''
    ...
    if not os.path.exists(filepath):
        ...
        filepath, _ = urllib.request.urlretrieve(DATA_URL, filepath, _process)
...
def _read_one_batch(path, is_random_label):
    '''
    This function takes the directory of one batch of data and returns the images and
    corresponding labels as numpy arrays
    :param path: the directory of one batch of data
    :param is_random_label: use random labels
...
def read_all_images(address_list, shuffle=True, is_random_label=False):
    '''
    ...
    :return: Data are in 4D arrays: [num_images,
    ...
    for address in address_list:
        print('reading image from ', address)
...
def whitening_image(image_np):
    '''
    per image whitening
    :param image_np: 4D numpy array representing  a batch of images
    :return: the image array after whitening
    '''
...
def random_crop_and_flip(batch_data, padding_size):
    '''
    random crop and random flip a batch of images
    :param batch_data: a 4D batch array
    :param padding_size: int. how many layers of 0 padding was added to each side
    :return: randomly cropped and flipped image
    '''

    cropped_batch = np.zeros([len(batch_data) * IMG_HEIGHT * IMG_WIDTH * IMG_DEPTH]).reshape(len(batch_data),
...
def prepare_train_data(padding_size):
    ...
    data, label = read_all_images(path_list, is_random_label=TRAIN_RANDOM_LABEL)
